...
std::optional<std::pair<int, int>> find_sum_pair(std::vector<int> &nums,
                                                 int desired_num) {
  int left = 0;
  int right = nums.size() - 1;

  // Sort the array.  Then we can just bring our window sizes inward if we are
  // greater or less than our target.
...
TEST(Day01, can_find_triplet) {
  ...
  EXPECT_EQ(std::get<0>(pair) * std::get<1>(pair) * std::get<2>(pair),
            241861950);
}
TEST(Day01, part_2) {
  auto nums = read_file("day01_part01.txt");
