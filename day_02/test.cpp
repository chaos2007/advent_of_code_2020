...
bool is_valid_password(const PasswordConfig &passwordConfig) {
    ...
    size_t count = std::count(password.begin(), password.end(), passwordConfig.character);
...
TEST(Day02, valid_password) {
    PasswordConfig config_1 = {.min = 1, .max = 3, .character='a', .password=std::string("abcde")};
    EXPECT_TRUE(is_valid_password(config_1));
    PasswordConfig config_2 = {.min = 1, .max = 3, .character='a', .password=std::string("abcde")};
    EXPECT_TRUE(is_valid_password(config_2));
}

TEST(Day02, under_min_invalid) {
    PasswordConfig config = {.min = 1, .max = 3, .character='a', .password=std::string("bbcde")};
    EXPECT_FALSE(is_valid_password(config));
}

TEST(Day02, over_max_invalid) {
    PasswordConfig config = {.min = 1, .max = 3, .character='a', .password=std::string("aaaae")};
    EXPECT_FALSE(is_valid_password(config));
}
...
TEST(Day02, valid_password_2) {
    ...
    PasswordConfig config_2 = {.min = 1, .max = 3, .character='b', .password=std::string("cdefg")};
    EXPECT_FALSE(is_valid_password_2(config_2));
    PasswordConfig config_3 = {.min = 2, .max = 9, .character='c', .password=std::string("ccccccccc")};
